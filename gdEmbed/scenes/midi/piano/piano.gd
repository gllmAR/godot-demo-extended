...
const WhiteKeyScene := preload("res://scenes/midi/piano/piano_keys/white_piano_key.tscn")
const BlackKeyScene := preload("res://scenes/midi/piano/piano_keys/black_piano_key.tscn")
...
func _ready() -> void:
	print("🎹 Piano starting up...")
	print("🎹 White keys container: ", white_keys)
	print("🎹 Black keys container: ", black_keys)
	...
	print("🎹 Creating keys from ", START_KEY, " to ", END_KEY)
	for i in range(START_KEY, END_KEY + 1):
		piano_key_dict[i] = _create_piano_key(i)

	print("🎹 Created ", len(piano_key_dict), " keys")
	print("🎹 White keys count: ", white_keys.get_child_count())
	print("🎹 Black keys count: ", black_keys.get_child_count())

	if white_keys.get_child_count() != black_keys.get_child_count():
		_add_placeholder_key(black_keys)
		print("🎹 Added placeholder key")
...
func _create_piano_key(pitch_index: int) -> PianoKey:
	...
	print("🎹 Creating key for pitch ", pitch_index, " (note index ", note_index, ")")
	
	if _is_note_index_sharp(note_index):
		print("🎹 Creating black key")
		piano_key = BlackKeyScene.instantiate()
		black_keys.add_child(piano_key)
	else:
		print("🎹 Creating white key")
	...
	print("🎹 Key created and set up: ", piano_key.name)
	return piano_key
